...
class Task1BillCounterTest {
    ...
    fun testTask3BillCounter1() {
        ...
        assertEquals(fairlySplit, counter.calculateFairlySplit(billArray, 0, 5).lowercase())
    }

    @Test
    fun testTask3BillCounter2() {
        val billArray = intArrayOf(3, 10, 2, 9, 18, 25)
        assertEquals(fairlySplit, counter.calculateFairlySplit(billArray, 3, 29).lowercase())
    }

    @Test
    fun testTask3BillCounter3() {
        val billArray = intArrayOf(3, 10, 2, 9)
        assertEquals("5", counter.calculateFairlySplit(billArray, 1, 12).lowercase())
    }

    @Test
    fun testTask3BillCounter4() {
        val billArray = intArrayOf(3, 10, 2, 9, 18, 25, 45)
        assertEquals("22", counter.calculateFairlySplit(billArray, 4, 69).lowercase())
    }
